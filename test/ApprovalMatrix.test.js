...
contract('ApprovalMatrix', ([deployer, investor1, investor2]) => {
  ...
  describe('TEST addEmployee() function', async () => {
    ...
    it('TEST EVENT EMITTED', async () => {
      ...
      assert.equal(event._employerAdress, investor1, '_account is correct')
      ...
      assert.equal(event._employeeRole, 'Engineer', '_employeeRole is correct')
...
//check struct
      ...
      assert.equal(employee.EmployerAdress, investor1, 'EmployerAdress is correct')
      ...
      assert.equal(employee.EmployeeRole, 'Engineer', 'EmployeeRole is correct')
